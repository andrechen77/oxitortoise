...
struct CodegenFnInfo {
    ...
    /// Associates each LIR register with the walrus local variable, if one
    /// exists. It might not exist if the value of a register is only used
    /// immediatelys after it is set.
    local_ids: HashMap<lir::Reg, wir::LocalId>,
}
...
struct CodegenFnCtx<'b> {
    ...
    /// The number of getters remaining for each LIR register.
    remaining_getters: HashMap<lir::Reg, usize>,
}
...
fn write_function_setup(
    ...
    let parameter_types: Vec<_> =
        func.parameters.iter().map(|param| translate_val_type(func.registers[*param].ty)).collect();
    let return_types: Vec<_> = func
        .return_values
        .iter()
        .map(|param| translate_val_type(func.registers[*param].ty))
        .collect();
    let mut function =
    ...
    let mut fn_info = CodegenFnInfo { compound_labels: HashMap::new(), local_ids: HashMap::new() };

    // allocate local variables for the function parameters
    let mut arg_locals = Vec::new();
    for var_id in func.parameters.iter() {
        ...
        fn_info.local_ids.insert(*var_id, wir_local_id);
...
fn allocate_local_for_var(
    ...
    let local_id = module_locals.add(translate_val_type(func.registers[var_id]));
