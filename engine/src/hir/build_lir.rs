//! These functions are used on a fully optimized HIR program to translate it
//! into LIR. No optimization is performed here. Each HIR function will
...
pub struct LirProgramBuilder {
    pub available_user_functions: HashMap<hir::FunctionId, lir::FunctionId>,
    pub function_signatures:
        HashMap<lir::FunctionId, (Vec<lir::ValType>, SmallVec<[lir::ValType; 1]>)>,
}

pub fn hir_to_lir<I: InstallLir>(
    hir: &hir::Program,
) -> (lir::Program, HashMap<hir::FunctionId, lir::FunctionId>) {
    ...
    for hir_fn_id in hir.functions.keys() {
        trace!("translating function signature for {}", hir_fn_id);
        let signature = translate_function_signature(hir, hir_fn_id);
        // allocate a new function id for the LIR function
        let lir_fn_id = user_function_tracker.insert(());
        builder.available_user_functions.insert(hir_fn_id, lir_fn_id);
    ...
    for hir_fn_id in hir.functions.keys() {
        trace!("translating function body for {}", hir_fn_id);
        let lir_fn = translate_function_body::<I>(hir, hir_fn_id, &mut builder);
        let lir_fn_id = builder.available_user_functions[&hir_fn_id];
        lir_fn_bodies.insert(lir_fn_id, lir_fn);
    }

    // add entrypoint shim functions
    let mut entrypoints = HashMap::new();
    for hir_fn_id in hir.functions.keys().filter(|id| hir.functions[*id].is_entrypoint) {
        let shim = create_entrypoint_shim(hir, hir_fn_id, &mut builder);
        if let Some(shim) = shim {
            // allocate a new function id for the shim function
            let lir_fn_id = user_function_tracker.insert(());
            lir_fn_bodies.insert(lir_fn_id, shim);
            entrypoints.insert(hir_fn_id, lir_fn_id);
        }
    }
...
pub struct LirInsnBuilder<'a> {
    ...
    /// The HIR function ID being translated.
    pub fn_id: hir::FunctionId,
    /// Maps HIR local variables to their LIR representations.
    pub local_to_lir: HashMap<hir::LocalId, LocalLocation>,
    ...
    pub node_to_lir: HashMap<hir::NodeId, SmallVec<[lir::ValRef; 1]>>,
    ...
    pub block_to_insn_seq: HashMap<hir::NodeId, lir::InsnSeqId>,
}
...
/// Describes how an HIR local variable is stored in LIR.
#[derive(Debug, Copy, Clone)]
pub enum LocalLocation {
    /// The HIR local variable is stored on the stack at the specified offset.
    Stack { offset: usize },
    /// The HIR local variable is stored as an LIR local variable, starting at
    /// the specified index.
...
fn translate_function_body<I: InstallLir>(
    ...
    let lir_function = lir::Function {
        ...
        is_entrypoint: false, // even if the HIR function was an entrypoint, the real entrypoint is a shim function created later
    };
