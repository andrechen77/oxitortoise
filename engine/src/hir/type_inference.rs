...
use crate::hir::{
    FunctionId, HirVisitor, LocalId, NlAbstractTy, Node as _, NodeId, NodeKind, Program,
    node::SetLocalVar, visit_hir_function,
};
...
pub fn narrow_types(program: &mut Program) {
    ...
    while changed {
        ...
        for fn_id in program.functions.keys() {
            visit_hir_function(&mut visitor, program, fn_id);
