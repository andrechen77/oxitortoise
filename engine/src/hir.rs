...
pub use build_lir::{LirProgramBuilder, hir_to_lir};
...
// TODO This sucks pls separate the two levels that HIR has melded together
#[derive(derive_more::Debug)]
...
pub fn visit_hir_function<V: HirVisitor>(visitor: &mut V, program: &Program, fn_id: FunctionId) {
    visit_node_recursive(visitor, program, fn_id, program.functions[fn_id].root_node);
}

fn visit_node_recursive<V: HirVisitor>(
    visitor: &mut V,
    program: &Program,
    fn_id: FunctionId,
    node_id: NodeId,
